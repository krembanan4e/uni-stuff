...
public class Main {
    public static void main(String[] args) {
        ...
        //za mapiranje na dostapniot materijal vo toj sho trebit da se transformirat
        Map<String, String> materijali = new HashMap<>();
        for (int i = 0; i < n; i++) {
            String pocheten = sc.next();
            String transformiran = sc.next();
            materijali.put(pocheten, transformiran);
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            String transformiran = sc.next();
            int cena = sc.nextInt();
            graph.addEdge(pocheten, transformiran, cena);
        }

        int sum = 0;

        for (String pocheten : materijali.keySet()){ 
            Map<String, Integer> distance = graph.shortestPath(pocheten);
            sum += distance.get(materijali.get(pocheten));
        }
